...
def extractRankingField(file, columnNames):
  reader = open(file)
  inputFile = csv.DictReader(reader)
  result ={}
...
def ProcessFinalData(user_data, college_rank):
   ...
   for i in range(size):
      ...
      try:
        ...
        result[name] =College_Info(rank, city, state, tuition, sat, accept_rate, debt, male_ratio)
      except Exception as ex:
        pass
   return result

def saveData(final_data) :
